...
export const HANDOFF_LOGIN_REQUIRED_NOTICE =
    '**Cloud Handoff Requires a December Account**\n\nCloud handoff migrates your local terminal session and workspace to December Cloud ([trydecember.com](https://trydecember.com)).\n\nTo proceed, please log in:\n- Type `/login` and select **Sign in with December** (Device Code), or\n- Run `december login` in your terminal'

export const HANDOFF_INSUFFICIENT_CREDITS_NOTICE =
    '**Insufficient Credits for Cloud Handoff**\n\nYour December Wallet balance is **$0.00**. Cloud handoff requires active credits to spin up remote cloud sandbox sessions on December Cloud.\n\n**Action Required:**\n- Add credits at: [https://trydecember.com/settings/billing](https://trydecember.com/settings/billing)\n- Once topped up, re-run `/handoff` to sync your workspace'

export const HANDOFF_SUCCESS_NOTICE = (sessionId: string) =>
    `**Workspace Handed Off Successfully!**\n\nYour workspace files and chat history have been synced to December Cloud:\n- **Cloud Session:** [https://trydecember.com/s/${sessionId}](https://trydecember.com/s/${sessionId})\n\n*Exiting local terminal in 3 seconds...*`
